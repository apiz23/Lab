...
int main(){
    ...
    for(int i = 0; i < max; i++){
        if(markStd[i] >= 85){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[0],achievement[0]);
            countGrade[0]++;
        }else if(markStd[i] >= 80){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[1],achievement[0]);
            countGrade[1]++;
        }else if(markStd[i] >= 75){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[2],achievement[0]);
            countGrade[2]++;
        }else if(markStd[i] >= 70){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[3],achievement[1]);
            countGrade[3]++;
        }else if(markStd[i] >= 65){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[4],achievement[1]);
            countGrade[4]++;
        }else if(markStd[i] >= 60){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[5],achievement[1]);
            countGrade[5]++;
        }else if(markStd[i] >= 55){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[6],achievement[2]);
            countGrade[6]++;
        }else if(markStd[i] >= 50){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[7],achievement[2]);
            countGrade[7]++;
        }else if(markStd[i] >= 45){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[8],achievement[2]);
            countGrade[8]++;
        }else if(markStd[i] >= 40){
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[9],achievement[3]);
            countGrade[9]++;
        }else{
            printf("%d \t\t%.2lf \t %s \t %s\n",i+1, markStd[i],grade[10],achievement[4]);
            countGrade[10]++;
        }
    }
