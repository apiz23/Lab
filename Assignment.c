...
int longTerm(int a){
    ...
    else if(a%24 == 0){return 60 + (a/24-7)*4;}
    else{return 60 + (a/24-6)*4;}
}
...
float shortTerm(float b){
    ...
    //less than 1 hour
    if(b < 1){
        if(b <= ((float)1/3)){return 0;} 
    	else if(b <= (float)2/3){return 2;}
        else{return 3;}
    }
    // less than 7 hour
    else if(b <= 6){return  ceil(b)*3;}
        
    //more than ONE day
    else if((int)ceil(b)%24 == 0){return (b/24)*18;}
    else if((int)ceil(b)%24 <= 6){return ((int)b/24)*18 + (int)ceil(b)%24*3;}
    else{return ((int)ceil(b/24))*18;}
}
